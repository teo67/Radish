namespace Tools.Operators {
    class Pop : SimpleOperator {
        ...
        public override IValue Run() {
            ...
            if(index < 0) {
                throw Error("Unable to remove from an array using an index less than zero!");
            }
            if(arr.Object.Count < index + 1) {
                throw Error("Unable to remove from an array with an index greater than or equal to that array's size!");
            }
            if(index == arr.Object.Count - 1) {
                ...
                for(int i = 0; i < arr.Object.Count; i++) {
                    if(arr.Object[i].Name != $"{i}") {
                        throw Error("The remove() and pop() functions can only be called on arrays!");
                    }
